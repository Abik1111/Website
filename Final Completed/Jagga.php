<?php
	...
	define('IMAGE_REQUEST_LOCATION', 'Images/Property_Request_Images');
	...
	define('REQUEST_TABLE', 'Property_table_request');
	...
	Class JaggaRetrieve{
		...
		public function echoHTMLDescription(){
			$copy_string = htmlspecialchars(strVal($this->description));
			$copy_string = str_replace("\n", '<br/>', $copy_string);
			echo $copy_string;
		}
	...
	Class JaggaSave{
		...
		public function saveFromRequest($id_in_request, $arg_location, $arg_area, $arg_price, $arg_description, $arg_owner_contact, $arg_images, $arg_jagga, $user_id, $delete_of_user){
			$data = array();
			$data[LOCATION] = strVal($_POST[$arg_location]);
			$data[AREA] = $_POST[$arg_area];
			$data[PRICE] = $_POST[$arg_price];
			...
			$Srchandler = new ImageHandler(IMAGE_REQUEST_LOCATION);
			$Dsthandler = new ImageHandler(IMAGE_LOCATION);
			if($_POST[$delete_of_user] != 1){
				$Dsthandler->copyFormDirectory($Srchandler, $id_in_request, $id);
			}
			$Dsthandler->saveImages($id, $arg_images);
			$Srchandler->deleteAllImages($id_in_request);
			return $id;
		}
	...
	Class JaggaBlock{
		...
		public function __construct($data, $index){
			...
			if(is_array($imageNames) && sizeof($imageNames)>0){
	...
	Class JaggaSelect{
		...
		public function getAllJagga($numberOfJagga='', $indexOfPage='', $jagga_type='all'){
			//selcting the type of properties
			$condition = null;
			if($jagga_type != 'all'){
				if($jagga_type == 'land'){
		...
		public function getAllPagesNo($numberOfJagga, $jagga_type='all'){
			//selcting the type of properties
			$condition = null;
			if($jagga_type != 'all'){
				if($jagga_type == 'land'){
		...
		public function getSearchedJagga($keyword, $numberOfJagga, $indexOfPage, $jagga_type='all'){
			//selcting the type of properties
			$conditions = null;
			if($jagga_type != 'all'){
				if($jagga_type == 'land'){
					$conditions = '('.IS_JAGGA.'=1) AND ';
				}
				else{
					$conditions = '('.IS_JAGGA.'=0) AND ';
				}
			}
		...
		public function getResultsNo($keyword, $jagga_type='all'){
			//selcting the type of properties
			$conditions = null;
			if($jagga_type != 'all'){
				if($jagga_type == 'land'){
		...
		//Returns the no. of page the result will have
		public function getSearchPagesNo($keyword, $numberOfJagga,$jagga_type='all'){
			$number = $this->getResultsNo($keyword,$jagga_type);
			$number /= $numberOfJagga;
			$number = ceil($number);
			return $number;
		}
		...
		public static function parseToJson($data, $fileName, $varName, $pages_num='', $current_page=''){
			$string = '{'."\n";
			$string .= '"'.$varName.'":[';
			if(is_array($data)){			
				for($i=0; $i<sizeof($data); $i++){
					...
					$string .= '"Price": "Rs.'.JaggaBlock::getMoneyFormat($data[$i]->getPrice()).'/-",'."\n";
	...
	//-------------------------------------------------------
	//This is replicaa for request handling

	//Blueprint of object where datas to be retrieved
	Class JaggaRequestRetrieve{
		private $id;//id of jagga
		private $location;//Location of ghar jagga
		private $area;//Area in m2
		private $price;//Price of ghar, jagga
		...
		public function loadFromDatabase($id){
			...
			$handler = new ImageHandler(IMAGE_REQUEST_LOCATION);
			$image_names = $handler->getImagesNames($id);
			$images = array();
			foreach($image_names as $name){
				$this->images[] = $handler->getSrc($id, $name);
	...
	Class JaggaRequestSave{
		...
		public function saveToDatabase(){
			...
			$data[AVAILABILITY] = 1;
			$data[OWNER_CONTACT] = $this->ownerContact;
			$id = $this->dataBase->insert('jagga_REQUEST_TABLE', $data);
			if($id == false){
				return false;
			}
			$handler = new ImageHandler(IMAGE_REQUEST_LOCATION);
		...
		public function saveData($arg_location, $arg_area, $arg_price, $arg_description, $arg_owner_contact, $arg_images, $arg_jagga, $user_id){
			...
			$data[IS_JAGGA] = ($_POST[$arg_jagga]==1)?1:0;
			$data[USER_ID] = $_POST[$user_id];
			$id = $this->dataBase->insert(REQUEST_TABLE, $data);
			if($id == false){
				//Error handling
				return false;
			}
		...
		public function updateData($id, $arg_location, $arg_area, $arg_price, $arg_description, $arg_owner_contact, $arg_images, $is_jagga, $delete_prev){
			...
			$data[OWNER_CONTACT] = strVal($_POST[$arg_owner_contact]);
			$data[IS_JAGGA] = ($_POST[$is_jagga]==1)?1:0;
			$this->dataBase->update(REQUEST_TABLE, $data, ID."=$id");
			$handler = new ImageHandler(IMAGE_REQUEST_LOCATION);
			$_POST[$delete_prev] = ($_POST[$delete_prev]==1)?1:0;
			if($_POST[$delete_prev] == 1){
				$handler->deleteAllImages($id);
	...
	Class JaggaRequestDelete{
		...
		public function delete($id){
			$condition = ID." = $id";
			$result = $this->dataBase->delete(REQUEST_TABLE, $condition);
			if($result){
				$handler = new ImageHandler(IMAGE_REQUEST_LOCATION);
				$handler->deleteAllImages($id);
			}
	...
	Class JaggaRequestSelect{
		...
		public function getAllJagga($numberOfJagga='', $indexOfPage='', $jagga_type='all'){
			...
			if(is_numeric($numberOfJagga) && is_numeric($indexOfPage)){
				$offset = $numberOfJagga*($indexOfPage-1);
				$data = $this->dataBase->select(REQUEST_TABLE, $fields, $condition, $order, $numberOfJagga, $offset);
			}
			else{
				$data = $this->dataBase->select(REQUEST_TABLE, $fields, $condition, $order);
				echo $this->dataBase->getErrors();
			}
			if(is_array($data)){
				$jaggas = array();
				for($i=0; $i<sizeof($data); $i++){ 
					$jaggas[] = new JaggaRequestBlock($data, $i);
				}
				return $jaggas;
			}
			else{
		...
		public function getAllPagesNo($numberOfJagga, $jagga_type='all'){
			...
			$fields = array(ID);
			$data = $this->dataBase->select(REQUEST_TABLE, $fields, $condition);
			$number = sizeof($data);
			$number /= $numberOfJagga;
			$number = ceil($number);
			return $number;
