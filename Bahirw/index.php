<?php
    ...
    $searching = false;
    $property_type = 'all';
    //In case of change in page number
    if(isset($_GET['page_no'])){
        $current_page = $_GET['page_no'];
    }
    //In case of search pressed
    if(isset($_GET['search_key'])){
        $data = $selector->getSearchedJagga($_GET['search_key'],DATA_NUM_IN_PAGE, $current_page, $property_type);
        $total_pages_no = $selector->getSearchPagesNo($_GET['search_key'],DATA_NUM_IN_PAGE);
        $search_result_no = $selector->getResultsNo($_GET['search_key']);
        $searching = true;
    }
    else{
        $data = $selector->getAllJagga(DATA_NUM_IN_PAGE, $current_page, $property_type);
        $total_pages_no = $selector->getAllPagesNo(DATA_NUM_IN_PAGE);
    }
    JaggaSelect::deleteJson('data/tab3.json');
    JaggaSelect::parseToJson($data, 'data/tab3.json', 'desired_properties', $total_pages_no);
?>
<!DOCTYPE html>
<html lang="en">
    <head>
