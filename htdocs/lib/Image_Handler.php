<?php
	class ImageHandler{
		...
		public function saveImages($imageFolder, $nameInForm){
			...
			if(is_array($_FILES[$nameInForm]['name'])){
		...
		public function copyFormDirectory($source, $sourceFolder, $storeFolder){
			$folder_src_name = strVal($sourceFolder);
			$folder_dst_name = strVal($storeFolder);
			
			//Making new directory inside images folder if doesn't exist
			if(!file_exists($this->relPath.$folder_dst_name)){
				mkdir($this->relPath.$folder_dst_name);	
			}
			
			if(file_exists($source->relPath.$folder_src_name)){
				$image_names = $source->getImagesNames($folder_src_name);
				//Deleting all images inside folder
				foreach($image_names as $name){
					copy($source->relPath.$folder_src_name.'/'.$name, $this->relPath.$folder_dst_name.'/'.$name);;
				}
			}
